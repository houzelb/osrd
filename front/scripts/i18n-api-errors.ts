...
async function run() {
  try {
    ...
    if (!('oneOf' in EditoastError)) throw new Error(`"EditoastError" is not a schema object`);
    if (!EditoastError.oneOf) throw new Error(`Expected "EditoastError" to be a "oneOf" object`);
